...
const finalizeVotes = () => {
  ...
  setEliminations(prev => [...prev, { 
    ...
    votes: votingResults,
    reasons: { ...votingReasons } // Make a copy to preserve the full state
  }]);
  ...
  const generateVotingPrompt = (llm, roundNumber, roundDiscussions) => {
    let prompt = `You are ${playerMap[llm.instanceId]}, voting on which participant's feedback was least valuable in the product evaluation.\n\n`;
    
    prompt += `Product: ${productName}\n\n`;
    
    prompt += `Here are the comments from all participating players:\n\n`;
    ...
    prompt += `\nBased on these comments, which player would you vote to eliminate? Provide your vote by starting with "I vote for [Player X]" and then explain your reasoning in 1-2 sentences. Be brief but specific about why you're voting for this player.`;
  ...
  const parseVote = (voteResponse, validIds) => {
    ...
    const voteMatch = voteResponse.match(/I vote for Player (\d+)/i);
            ...
            <div key={voterId} className="p-5 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-center mb-3">
                <span className={`inline-flex items-center justify-center h-10 w-10 rounded-full ${
                  voter.stance === 'pro' 
                    ? stanceColors.pro.accent 
                    : voter.stance === 'against' 
                      ? stanceColors.against.accent 
                      : stanceColors.neutral.accent
                } text-sm font-medium mr-3`}>
                  {voter.avatarText || voter.name?.charAt(0) || '?'}
                </span>
                <div>
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {Array.isArray(winner) ? (
              winner.map(w => (
                <div key={w.instanceId} className="flex flex-col items-center p-6 bg-white rounded-lg shadow-md">
                  <span className={`inline-flex items-center justify-center h-16 w-16 rounded-full ${
                    ...
                    {playerMap[w.instanceId]?.split(' ')[1] || w.name.charAt(0)}
                  </span>
              ...
              <div className="flex flex-col items-center p-6 bg-white rounded-lg shadow-md">
                <span className={`inline-flex items-center justify-center h-20 w-20 rounded-full ${
                  winner.stance === 'pro'
                    ? stanceColors.pro.accent
                    : winner.stance === 'against'
                      ? stanceColors.against.accent
                      : stanceColors.neutral.accent
                } text-2xl font-medium mb-3`}>
                  {playerMap[winner.instanceId]?.split(' ')[1] || winner.name.charAt(0)}
                </span>
